        ...
        for (var z = 0; z < stacks; z++) {
            if (z < stacks - 1) {
                for (var zs = z + 1; zs < stacks; zs++) {
                    var prev = sheet[z];
                    sheet[z] = clipSubtract(prev, sheet[zs]);
                    prev.remove();
                }
            }
        }
...
function join(z,s){
    var prev = sheet[z];
    var savedStyle = prev.style;
    sheet[z] = clipUnite(s, prev);
...
function solid(z){
    ...
    var prev = sheet[z];
    var savedStyle = prev.style;
    sheet[z] = clipUnite(prev, outsideframe);
    sheet[z].style = savedStyle;
    prev.remove();
    outsideframe.remove();
}
...
function frameIt(z){
        ...
        var prev = sheet[z];
        var savedStyle = prev.style;
        sheet[z] = clipIntersect(outsideframe, prev);
        sheet[z].style = savedStyle;
        outsideframe.remove();
        prev.remove();
